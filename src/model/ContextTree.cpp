      ...
      if(_all_context[i]->getParent() >= 0)
        {
          ...
          if((_all_context[i] != NULL) && (!_all_context[i]->isLeaf()))
            for(int  m = 0; m < (int)_alphabet_size; m++)
  ...
  for(int i = 0; i  < (int)_all_context.size(); i++)
    {
      ...
      for(int l = 0; l < (int)_alphabet_size; l++){
        probs[l] = log((double)((_all_context[i]->getCounter())[l])/total);
      }
  ...
  for(int i = 0; i  < (int)_all_context.size(); i++)
    {
      ...
      for(int l = 0; l < (int)_alphabet_size; l++){
          ...
          probs[l] = log((double)((_all_context[i]->getCounter())[l] + pseudocount*prob)/(total + pseudocount));
      }
...
void ContextTree::initializeCounter(const std::vector<Sequence> & sequences, int order, const std::vector<double> &weights)
  ...
  void ContextTree::pruneTree(double delta)
  {
    ...
    std::vector<int> nodesToPrune (x.begin(),x.end());
    std::set<int>::iterator it;
    double small_ = ((double)_alphabet_size)*log(sample_size);
