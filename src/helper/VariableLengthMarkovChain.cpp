...
tops::model::VariableLengthMarkovChainPtr createMachlerVLMC() {
  ...
  root->setDistribution(tops::model::DiscreteIIDModel::make(
    std::vector<Probability>{ log(0.50), log(0.50) }));

  auto c0 = tree->createContext();
  root->setChild(c0, 0);
  c0->setDistribution(tops::model::DiscreteIIDModel::make(
    std::vector<Probability>{ log(0.20), log(0.80) }));

  auto c1 = tree->createContext();
  root->setChild(c1, 1);
  c1->setDistribution(tops::model::DiscreteIIDModel::make(
    std::vector<Probability>{ log(0.21), log(0.79) }));

  auto c10 = tree->createContext();
  c1->setChild(c10, 0);
  c10->setDistribution(tops::model::DiscreteIIDModel::make(
    std::vector<Probability>{ log(0.22), log(0.78) }));

  auto c11 = tree->createContext();
  c1->setChild(c11, 1);
  c11->setDistribution(tops::model::DiscreteIIDModel::make(
    std::vector<Probability>{ log(0.25), log(0.75) }));

  auto c100 = tree->createContext();
  c10->setChild(c100, 0);
  c100->setDistribution(tops::model::DiscreteIIDModel::make(
    std::vector<Probability>{ log(0.30), log(0.70) }));

  auto c101 = tree->createContext();
  c10->setChild(c101, 1);
  c101->setDistribution(tops::model::DiscreteIIDModel::make(
    std::vector<Probability>{ log(0.10), log(0.90) }));
